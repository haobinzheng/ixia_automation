...
portList = [[ixChassisIpList[0], 1, 1], [ixChassisIpList[0], 2, 4]]
...
try:
    ...
    ethernetDstField.StartValue = "FF:FF:FF:FF:FF:FF"
    ...
    # IPv6
    ipv6FieldObj = createPacketHeader(rawTrafficItemObj, packetHeaderToAdd='^ipv6$', appendToStack='^vlan$')
    ipv6SrcField = ipv6FieldObj.find(DisplayName='Source Address')
    ipv6SrcField.ValueType = 'increment'
    #ipv6SrcField.ValueType = 'Single'
    ipv6SrcField.StartValue = '2001::1'
    ipv6SrcField.StepValue = '::1'
    ipv6SrcField.CountValue = 1
    
    print(f"ipv6SrcField = {ipv6SrcField}")
    ...
    ipv6DstField.ValueList = ['2002::1']    
    #ipv6DstField = '2002::1'
    print(f"ipv6DstField = {ipv6DstField}")
    ...
    udpFieldObj = createPacketHeader(rawTrafficItemObj, packetHeaderToAdd='^udp$', appendToStack='^ipv6$')
