...
######################################
# reset all test cases at beginning
######################################
...
#######################################
#  Input which test cases to excute
#######################################
while True:
	num = input_test_cases() 
	if num == 0: 
		flag_vc = 1                    #  1-- flap vc and next hops
		flag_vs_attach = 1             #  2 --add and remove ports (AC) from virtual switches
		flag_flush = 1                 #  3 --flush mac table and flap mac learning
		flag_lsp_switch = 1            #  4 --LSP switch over
		flag_hw_bfd = 1                #  5 - change sw bfd to hw bfd
		flag_pw_failover = 1           #  6 - node failure trigger pw switch over
		flag_lsp_add_drop = 1          #  7 - Add drop lsp
		flag_max_telnet = 1            #  8 - max telnet
		flag_swbfd_profiles = 1        #  9 - change sw bfd timers
		flag_pw_switch = 1             #  10 - manual pw switch over
	elif num == 1:
		flag_vc = 1                    # flap vc and next hops
	elif num == 2:
		flag_vs_attach = 1             # add and remove ports (AC) from virtual switches
	elif num == 3:
		flag_flush = 1                 # flush mac table and flap mac learning
	elif num == 4:
		flag_lsp_switch = 1            #  4 --LSP switch over
	elif num == 5:
		flag_hw_bfd = 1                #  5 - change sw bfd to hw bfd
